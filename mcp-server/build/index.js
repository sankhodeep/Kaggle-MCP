...
class KaggleServer {
    ...
    async handleListNotebooks(args) {
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to list notebooks: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async handleGetNotebook(args) {
        const { notebookRef } = args;
        if (!notebookRef) {
            throw new McpError(ErrorCode.InvalidParams, 'notebookRef parameter is required');
        }
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get notebook: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async handleUpdateNotebook(args) {
        const { notebookRef, filePath } = args;
        if (!notebookRef || !filePath) {
            throw new McpError(ErrorCode.InvalidParams, 'Both notebookRef and filePath parameters are required');
        }
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to update notebook: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async handleRunNotebook(args) {
        const { notebookRef } = args;
        if (!notebookRef) {
            throw new McpError(ErrorCode.InvalidParams, 'notebookRef parameter is required');
        }
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to run notebook: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    setupErrorHandlers() {
